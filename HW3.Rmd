...
```{r mean tmax and tmin in each year, message = FALSE}
noaa_tmax =
  noaa %>% 
  filter(!is.na(tmax))%>% 
  group_by(year) %>% 
...
noaa_min = 
  ...
  group_by(year) %>% 
  summarize(
    average_tmin = round(mean(tmin, na.rm = TRUE), 2)) 
```
...
tmin_plot = 
  noaa_min %>% 
  ggplot(aes(x = year, y = average_tmin, color = id)) +
  geom_point(alpha = 0.3) +
  labs(
    x = "Years",
    y = "Average minumum temperature",
    color = "Station",
